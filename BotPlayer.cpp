...
float BotPlayer::evaluateState(const SimState &state) {
  ...
  float shellBonus =
      static_cast<float>(state.totalShells) * 0.5f; // More options multiplier.
...
std::pair<SimState, SimState> BotPlayer::simulateAction(const SimState &state,
                                                        Action action) {
...
float BotPlayer::expectiMiniMax(const SimState &state, int depth,
                                bool maximizingPlayer) {
  if (depth == 0 || state.player1Health <= 0 || state.player2Health <= 0 ||
      state.totalShells <= 0)
  ...
  if (maximizingPlayer) {
    ...
    for (int actionIndex = 0; actionIndex < 2; ++actionIndex) {
      ...
      const float probabilityOfLiveShell =
          (state.totalShells > 0)
      ...
      if (probabilityOfLiveShell == 0.0f) {
        ...
        float branchValue =
            expectiMiniMax(outcomes.second, depth - 1,
                           flip ? !maximizingPlayer : maximizingPlayer);
        ...
        bool flip = true;
        float branchValue =
            expectiMiniMax(outcomes.first, depth - 1,
                           flip ? !maximizingPlayer : maximizingPlayer);
        bestValue = std::max(bestValue, branchValue);
        continue;
      }
      ...
      float blankValue =
          expectiMiniMax(blankState, depth - 1,
                         flipBlank ? !maximizingPlayer : maximizingPlayer);
      float actionValue = probabilityOfLiveShell * liveValue +
                          probabilityOfBlankShell * blankValue;
    ...
    for (int actionIndex = 0; actionIndex < 2; ++actionIndex) {
      ...
      if (probabilityOfLiveShell == 0.0f) {
        ...
        auto outcomes = simulateAction(state, action);
        float branchValue =
            expectiMiniMax(outcomes.first, depth - 1, !maximizingPlayer);
        bestValue = std::min(bestValue, branchValue);
        continue;
      }
...
Action BotPlayer::chooseAction(const Shotgun &currentShotgun) {
  ...
  for (int actionIndex = 0; actionIndex < 2; ++actionIndex) {
    ...
    float blankVal = expectiMiniMax(blankState, searchDepth - 1,
                                    action == Action::SHOOT_SELF);

    float actionValue =
        liveShellProbability * liveVal + blankShellProbability * blankVal;
