...
| The victim of a restraint taken with Adrenaline | The first legal target that is not the seat it was taken from (`engine/Rules.cpp:233-239`) | none | With three or more seats, a real player would choose the victim. The engine picks one, so the value of stealing a restraint is a lower bound in a four-seat position |
