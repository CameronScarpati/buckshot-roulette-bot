...
private:
  /**
   * @brief This function evaluates the state created in the expectiMiniMax
   * search.
  ...
  /**
   * @brief This function changes the state according to an action and branches
   * the outcomes.
   * @param state Recursive game state.
   * @param action The action to perform.
   * @return Pair<SimState, SimState> (Live/Blank).
   */
  static std::pair<SimState, SimState> simulateAction(const SimState &state,
                                                      Action action);

  /**
   * @brief This function gives a value to the action it believes it should
   * perform.
   * @param state Recursive game state.
...
public:
  ...
  /**
   * @brief This constructor creates a bot player object with name, health and
   * opponent.
   * @param name String (Bot player identifier).
   * @param health Integer (Health amount).
   */
